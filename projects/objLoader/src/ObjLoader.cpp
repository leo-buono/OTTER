...
void ObjLoader::LoadMesh(const std::string& fileName, bool usingMaterial)
{
	...
	if (usingMaterial)
	{
		...
		m_shader->LoadShaderPartFromFile("shaders/mat_vertex_shader.glsl", GL_VERTEX_SHADER);
		m_shader->LoadShaderPartFromFile("shaders/mat_frag_shader.glsl", GL_FRAGMENT_SHADER);
	...
	while (std::getline(file, line))
	{
		...
		else if (line[0] == 'o')
		{
			...
			if (vectorIndex > 0)
			{
				...
				interleaved_vbo->LoadData(interleaved, size);
				...
				m_numberOfVertices.push_back(currentIndex);
